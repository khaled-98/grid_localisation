...
double MeasurementModel::getScanProbability(const geometry_msgs::Pose &curr_pose, const sensor_msgs::LaserScanConstPtr &scan)
{
	...
	for(int i=0; i<scan->ranges.size(); i+=beams_to_skip)
	{
		if(scan->ranges[i]<z_max && scan->ranges[i]>z_min) // within sensor range
		{
			...
			double weight = z_hit_* prob + (z_rand_/z_max);
			q *= weight;
		}
	}
	return q;
...
void MeasurementModel::setMap(const nav_msgs::OccupancyGrid &map)
{
    map_ = map;
    ROS_INFO("Computing likelihood field...");
	computeLikelihoodField(map);
	ROS_INFO("Likelihood field computed!");
}

void MeasurementModel::setLaserPose(const geometry_msgs::TransformStamped &laser_pose)
{
    laser_pose_ = laser_pose;
}

void MeasurementModel::computeLikelihoodField(const nav_msgs::OccupancyGrid &map)
{
...
void MeasurementModel::DFS(const int &index_curr, const int &index_of_obstacle, std::vector<bool> &visited)
{
